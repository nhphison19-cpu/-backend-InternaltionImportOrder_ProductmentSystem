...
async function createEmployee({ name, email, password, department, role }) {
  ...
  const employee = await prisma.employee.create({
    data: { name, email, password : passwordHash, department, role: role || 'STAFF' },
...
async function listEmployees({ page = 1, limit = 20, department, role, search }) {
  const where = {
...
async function updateEmployee(id, { name, department, role }) {
  await getEmployeeById(id); // throws 404 if missing
